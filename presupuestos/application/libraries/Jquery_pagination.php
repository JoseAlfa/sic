...
class Jquery_pagination{
	...
	var $uri_segment		= 3;
	var $full_tag_open		= '';//abre Contenedor principal
	var $full_tag_close		= '';//Cierra Contenedor principal
	var $first_tag_open		= '';
	var $first_tag_close	= '&nbsp;';
	var $last_tag_open		= '&nbsp;';
	var $last_tag_close		= '';
	...
	//Added By Jose Jimenez
	var $segmento_pagina	= 0;///Usada paga mantener el sgmento en caso de ser necesario actualizar por ajax la tabla en el segmento de donde se solicita la actualización
	var $id_complmet		='';//Usada para establecer un id personalizado al elemto que contiene el segmento
   ...
   var $show_count      = true;
	...
	function create_links()
	{
      ...
      if ($this->show_count)
      {
         ...
         $info = 'Viendo del ' . ( $curr_offset + 1 ) . ' al ' ;
		...
		//Asigna un id al input hidden para acceder a el por medio de javascript
		if ($this->id_complmet=='') {
			$this->id_complmet='segmento_pagina';
		}

		return $output.' <!--Con esto puedes usar el segmento en el que se encuentra la paginación--><input type="hidden" id="'.$this->id_complmet.'" value="'.$this->segmento_pagina.'">';		
	}
